...
def create_app() -> Flask:
    ...
    class RTPlayer:
        sid: str
        name: str
        score: int = 10000  # capital Ã  miser (jetons)
        choice: Optional[str] = None
        is_correct: Optional[bool] = None
        eliminated: bool = False  # DÃ©faite totale (capital=0)
    ...
    class RealtimeLobby:
        ...
        def start_game(self, questions: list) -> None:
            with self.lock:
                ...
                for p in self.players.values():
                    ...
                    p.eliminated = False  # Reset statut Ã©liminÃ©
                    p.choice = None
                    p.is_correct = None
                    p.bets = {"A": 0, "B": 0, "C": 0, "D": 0}
                # Le host dÃ©clenche explicitement le lancement de question
                self.phase = "waiting"
        ...
        def launch_question(self) -> None:
            with self.lock:
                ...
                for p in self.players.values():
                    p.choice = None
                    p.is_correct = None
                    # Ne rÃ©initialiser les mises que si le joueur n'est pas Ã©liminÃ©
                    if not p.eliminated:
                        p.bets = {"A": 0, "B": 0, "C": 0, "D": 0}
                self.phase = "question"
        ...
        def all_players_bet(self) -> bool:
            ...
            with self.lock:
                if self.phase != "question":
                    return False
                for p in self.players.values():
                    # Si p.bets est vide ou toutes les mises sont Ã  0, le joueur n'a pas misÃ©
                    if not p.bets or sum(p.bets.values()) == 0:
                        return False
                return len(self.players) > 0  # Au moins un joueur et tous ont misÃ©
        ...
        def validate(self) -> None:
            with self.lock:
                ...
                for p in self.players.values():
                    # Ignorer les joueurs dÃ©jÃ  Ã©liminÃ©s
                    if p.eliminated:
                        p.is_correct = None
                        continue
                    
                    # Si le joueur n'a pas misÃ© (p.bets vide ou tout Ã  0), il perd tout son capital
                    if not p.bets or sum(p.bets.values()) == 0:
                        p.score = 0
                        p.is_correct = False
                    else:
                        correct_bet = p.bets.get(self.correct, 0)
                        # Le joueur garde UNIQUEMENT sa mise correcte (les jetons non misÃ©s sont perdus)
                        p.score = correct_bet
    ...
    def _ws_host_start(payload):
        ...
        # Initialiser le jeu ET lancer automatiquement la premiÃ¨re question
